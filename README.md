...
- **Smooth Animations**: High-performance scroll and layout animations via GSAP.
- **Glassmorphism & Parallax**: Premium UI effects that give a modern feel.
...
2. **Navigate to the template you want**:
   ```bash
   cd portfolio_1_futuristic
   ```
3. **Open `index.html`** in your favorite browser. No build steps needed! Alternatively, run it via a local live server (e.g., VSCode Live Server).
4. **Customize**: Edit the text, update image placeholder URLs with your personal assets, and adjust the CSS variables defined at the top of the `style.css` files.
